...
SPIKES_CSV = PROJECT_ROOT / "scratch" / "detected_spikes.csv"
OUTPUT_CSV = PROJECT_ROOT / "scratch" / "recommendations.csv"
DELAY_BETWEEN_CALLS = 5  # seconds, stay well under daily/per-minute limits
RECOMMENDATION_COLUMNS = [
    "app_name", "category", "period", "granularity",
    "count", "baseline", "pct_change", "brief", "evidence_review_ids",
]
...
def write_empty_results():
    pd.DataFrame(columns=RECOMMENDATION_COLUMNS).to_csv(OUTPUT_CSV, index=False)


def local_recommendation(row, reviews):
    """Return a useful quota-free brief based only on the detected spike."""
    evidence = reviews[0]["content"][:160] if reviews else "the available review sample"
    return (
...
def generate_recommendations():
    ...
    try:
        ...
        write_empty_results()
        print("No spikes found; no recommendations generated.")
        return
    ...
    for _, row in spikes.iterrows():
        ...
        sample_text = "\n".join(f"- {r['content'][:200]}" for r in reviews) if reviews else "(no sample text available)"
        ...
        results.append({
            **row.to_dict(),
            "brief": brief,
            "evidence_review_ids": json.dumps([review["review_id"] for review in reviews]),
        })
